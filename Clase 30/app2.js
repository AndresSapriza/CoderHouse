...
if(cluster.isPrimary){
    ...
    app.use('/', viewRouter);
    app.use('/api/randoms', processRouter);
    ...
    console.log(`app primary process${process.pid}`);

    
    cluster.fork();
}else{
    appRandom.listen(8081, () => {
        console.log(`appRandom Listening on port 8081, by process${process.pid}`);
    });

    appRandom.use('/api/randoms', processRouter);
}